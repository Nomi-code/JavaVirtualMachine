...
class ByteCodeReader {
  ...
  public:
    ...
    void read_u4(jvm_raw_type::u4* const);
    void read_u8(jvm_raw_type::u8* const);
};

inline void ByteCodeReader::read_u1(jvm_raw_type::u1* const u1_addr) {
    this->file_binary_resource.read(reinterpret_cast<char*>(u1_addr), sizeof(jvm_raw_type::u1));
}
...
inline void ByteCodeReader::read_u4(jvm_raw_type::u4* const u4_addr) {
    this->file_binary_resource.read(reinterpret_cast<char*>(u4_addr), sizeof(jvm_raw_type::u4));
    if constexpr (std::endian::native == std::endian::little) {
        *u4_addr = __builtin_bswap32(*u4_addr);
    }
}

inline void ByteCodeReader::read_u8(jvm_raw_type::u8* const u8_addr) {
    this->file_binary_resource.read(reinterpret_cast<char*>(u8_addr), sizeof(jvm_raw_type::u8));
    if constexpr (std::endian::native == std::endian::little) {
